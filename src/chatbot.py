...
class MarineChatbot:
    ...
    * ``LLMMode.LOCAL`` — uses a locally running Ollama instance.
    ...
    def setup(self) -> None:
        ...
        chunks = load_and_split(self.data_dir, include_spreadsheets=False)
        ...
        if table_backend == "mysql":
            try:
                ...
                if table_store.needs_ingest(self.force_rebuild):
                    table_docs = load_table_documents(self.data_dir)
                    table_store.ingest_documents(
                        table_docs,
                        force_rebuild=self.force_rebuild,
                    )
            except Exception:
                logger.exception(
                    "MySQL table store setup failed; falling back to in-memory store."
                )
                table_docs = load_table_documents(self.data_dir)
            ...
            table_store = TableStore(table_docs) if table_docs else None
        graph_store_path = (
            str(Path(self.store_path) / "knowledge_graph")
            if self.store_path
            else config.KG_STORE_PATH
        )
    ...
    def chat(self, question: str) -> dict:
        ...
        table_docs = result.get("table_docs", [])
        ...
        return {
            "answer": answer,
            "sources": format_source_documents(context_docs + table_docs),
